...
def xmaslight():
    ...
    fireplace_lights(pixels, coords)

    return 'DONE'


def fireplace_lights(pixels, coords):
    cyl_coords_set = []
    max_z = -10000
    min_z = 10000
    ...
    # GRB Colors
    colors = [[0, 30, 0], [10, 30, 0], [20, 30, 0], [30, 30, 0]]
    rot_bins = np.linspace(0, 360, 36)
    # Scale for plot visualization, probably too high for LEDs
    if (USE_PLOT):
        colors = np.array(colors) * 8
    # Separate heights into discrete rings
    base_height = (max_z - min_z) / 3
    ...
    while(True):
        ...
        # Update
        # Determine heights of rotation bins
        for rot in range(len(rot_bins)):
            rot_bins[rot] = random.random() * base_height + base_height

        for c in cyl_coords_set:
            # Get rotational bin
            rot_bin = math.floor(((c.theta + np.pi)/2.0 /
                                  np.pi) * (len(rot_bins) - 1))

            slope = (rot_bins[(rot_bin+1) % len(rot_bins)] -
                     rot_bins[rot_bin]) / (np.pi / 18)
            h = slope * (c.theta + np.pi - (rot_bin * np.pi/18)) + \
                rot_bins[rot_bin]
            ...
            color_idx = round(((c.z - min_z) / h) * (len(colors) - 1))
            c.color = colors[color_idx]
